 ...
 *
 * Tests:
 * 1. Pawn may move 2 cells forward from its true starting row (row 6 for
 *    white - one row in from the board's edge row; see
 *    MoveValidationService). The edge row (row 7) is the back rank, not a
 *    pawn's own starting rank, so a pawn sitting there is not eligible for
 *    a double-step.
...
public class Iteration10_PawnMovementAndPromotionTest {
    ...
    public void testPawnDoubleMoveFromStartRow() {
        // Place white pawn at its true starting row (6,0) - one row in from
        // the board edge (row 7 is the back rank, not a pawn's start square)
    ...
    public void testPawnDoubleMoveBlockedByPieceInPath() {
        // Place pawn at its true starting row (6,0) and a blocking piece at middle (5,0)
        board.setPiece(6, 0, new Piece(Piece.Color.WHITE, Piece.Type.PAWN));
    ...
    public void testPawnDoubleMoveBlockedByActiveReservation() {
        // Pawn at its true starting row (6,0)
        board.setPiece(6, 0, new Piece(Piece.Color.WHITE, Piece.Type.PAWN));
